...
(function () {
  ...
  // ── Settings panel ────────────────────────────────────────────────────────
  // Injected before the [class*="graphIcon"] row inside .descriptions —
  // the exact location the red box points to (above "Main News / Attacking…").
  // Works on desktop, tablet and mobile because that DOM node exists on all
  // platforms whenever the faction page is open.
  ...
  function mountSettingsPanel(panel) {
    if (panel.isConnected) return; // already in place

    // Target: the parent of [class*="graphIcon"] inside .descriptions
    // This is the row that holds "Main News | Attacking | Funds…" sub-tabs —
    // inserting before it puts the accordion in the red box location.
    const graphContainer = document.querySelector(
      '#faction_war_list_id [class*="graphIcon"], ' +
      '.descriptions [class*="graphIcon"]'
    );
    if (graphContainer?.parentNode) {
      graphContainer.parentNode.insertBefore(panel, graphContainer.parentNode.firstChild);
      return;
    }

    // Fallback 1: after #faction_war_list_id (war tab, any device)
    const warList = document.getElementById("faction_war_list_id");
    if (warList?.parentNode) {
      warList.after(panel);
      return;
    }

    // Fallback 2: append to #factions so it's always accessible
    const factions = document.getElementById("factions");
    if (factions) factions.appendChild(panel);
  }

  function injectSettingsPanel() {
    if (settingsPanelInjected) {
      // Already created — re-mount if detached after tab switch / DOM rebuild
      const panel = document.getElementById("twi-settings-details");
      if (panel && !panel.isConnected) mountSettingsPanel(panel);
      return;
    }
    // Need at least #factions in the DOM before we can do anything

...
(function () {
  ...
  function mountSettingsPanel(panel) {
    if (panel.isConnected) return; // already mounted, nothing to do

    // 1st: right after the TWSE settings panel — desktop always hits this path,
    //      behaviour unchanged from what was working
    const twsePanel = document.querySelector("twse-settings-panel");
    if (twsePanel?.parentNode) {
      twsePanel.after(panel);
      return;
    }
    // 2nd: after #faction_war_list_id — War tab on any device
    const warList = document.getElementById("faction_war_list_id");
    if (warList?.parentNode) {
      warList.after(panel);
      return;
    }
    // 3rd: append to #factions — any other tab (Rank, Info, etc.) on tablet/mobile
    //      so users can set their API key without needing to be on the War tab
  ...
  function injectSettingsPanel() {
    if (settingsPanelInjected) {
      // Already created — re-mount if detached after a tab switch / DOM rebuild

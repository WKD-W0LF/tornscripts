...
(function () {
  ...
  const state = {
    ...
    // enabled defaults ON if an API key is already saved, otherwise OFF
    // so fresh installs start disabled until the user ticks the checkbox
  ...
  // Remove all injected call controls from the DOM (used when disabling)
  function removeAllControls() {
    document.querySelectorAll(".twi-call-control").forEach((el) => el.remove());
    document.querySelectorAll(".members-list li.enemy").forEach((li) => {
      li.classList.remove("twi-priority-row", "twi-assist-row");
    });
  }
  ...
  // ── Toggle checkbox (mirrors TWSE Sort style) ──────────────────────────────
  function ensureToggle() {
    if (document.getElementById("twi-calls-toggle-wrap")) return;
    if (!isWarPage()) return;

    // Try to anchor next to "TWSE Sort" checkbox, or fall back to the war header area
    const twseLabel = Array.from(document.querySelectorAll("label")).find(
      (el) => el.textContent.trim().startsWith("TWSE Sort")
    );
    const anchor = twseLabel?.parentElement || document.querySelector(".faction-war-wrap") || null;
    if (!anchor) return;

    const wrap = document.createElement("span");
    wrap.id = "twi-calls-toggle-wrap";

    const cb = document.createElement("input");
    cb.type = "checkbox";
    cb.id = "twi-calls-toggle";
    cb.checked = state.enabled;

    const lbl = document.createElement("label");
    lbl.htmlFor = "twi-calls-toggle";
    lbl.textContent = " TWI Calls";

    wrap.appendChild(cb);
    wrap.appendChild(lbl);

    // Insert after the TWSE label's parent, or prepend to anchor
    if (twseLabel) {
      twseLabel.parentElement.insertAdjacentElement("afterend", wrap);
    } else {
      anchor.prepend(wrap);
    }
    ...
    cb.addEventListener("change", async () => {
      ...
      if (state.enabled) {
        ...
        if (!ok) {
          // Auth failed or cancelled — turn it back off
          setEnabled(false);
          cb.checked = false;
          updateToggle();
          return;
  ...
  function updateToggle() {
    const cb = document.getElementById("twi-calls-toggle");
  ...
  GM_addStyle(`
    /* ── Toggle checkbox (matches TWSE Sort style) ── */
    #twi-calls-toggle-wrap{
      display:inline-flex;align-items:center;
      margin-left:10px;font-size:12px;cursor:pointer;
      font-family:Arial,sans-serif;vertical-align:middle
    }
    #twi-calls-toggle-wrap label{cursor:pointer;padding-left:4px;user-select:none}
    ...
    .twi-call-main{
      ...
      padding:1px 5px;background:rgba(20,20,20,.82);color:#fff;white-space:nowrap;font-size:10px
    }
    .twi-state-dot{
    ...
    /* ── Mobile (up to 900px): .member becomes flex column, control flows below name ── */
    @media(max-width:900px){
      ...
      .twi-call-actions[hidden]{display:none!important}
      .twi-flag{width:44px!important;min-width:44px!important;height:44px!important;padding:0!important;border-radius:6px!important}
      .twi-flag-icon{width:18px!important;height:18px!important}
      #twi-calls-toggle-wrap{
        display:flex!important;padding:6px 2px!important;font-size:14px!important
      }
      #twi-calls-toggle{width:20px!important;height:20px!important}
      #twi-faction-calls-status{

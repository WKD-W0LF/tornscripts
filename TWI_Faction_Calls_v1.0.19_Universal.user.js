...
(function () {
  ...
  function mountSettingsPanel(panel) {
    // Try anchors in priority order — first one found wins
    // 1. After twse-settings-panel (desktop: TWSE already mounted)
    const twsePanel = document.querySelector("twse-settings-panel");
    if (twsePanel) {
      if (!panel.isConnected || panel.previousElementSibling !== twsePanel) {
        twsePanel.after(panel);
      }
      return;
    }
    // 2. After #faction_war_list_id (all platforms when faction tab is active)
    const warList = document.getElementById("faction_war_list_id");
    if (warList) {
      if (!panel.isConnected || panel.previousElementSibling !== warList) {
        warList.after(panel);
      }
      return;
    }
    // 3. Append to body as absolute last resort so it's always reachable
  ...
  function injectSettingsPanel() {
    // Re-mount on every call if panel exists but got detached
    const existing = document.getElementById("twi-settings-details");
    if (existing) {
      mountSettingsPanel(existing);
      return;
    }
    if (settingsPanelInjected) return; // created but removed from DOM entirely — recreate below
    // Retry until body is ready

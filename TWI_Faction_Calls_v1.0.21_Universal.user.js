...
(function () {
  ...
  function mountSettingsPanel(panel) {
    // 1. After twse-settings-panel — desktop and any platform where TWSE loaded
    const twsePanel = document.querySelector("twse-settings-panel");
    if (twsePanel?.isConnected) {
      if (panel.previousElementSibling !== twsePanel) twsePanel.after(panel);
      return true;
    }
    // 2. After #faction_war_list_id — present on faction page on all platforms
    const warList = document.getElementById("faction_war_list_id");
    if (warList?.isConnected) {
      if (panel.previousElementSibling !== warList) warList.after(panel);
      return true;
    }
    // 3. Inside #factions container
    const factions = document.getElementById("factions");
    if (factions?.isConnected) {
      if (!panel.isConnected) factions.appendChild(panel);
      return true;
    }
    // Not ready yet — return false so retry loop keeps trying
    return false;
  }

  // The single panel instance, created once
  let _settingsPanel = null;
  let _mountRetryTimer = null;
  ...
  function injectSettingsPanel() {
    ...
    const mounted = mountSettingsPanel(_settingsPanel);
    updateSettingsPanel();

    // If not mounted in a good location yet, keep retrying every 300ms
    // until it succeeds (handles TornPDA slow DOM, TWSE loading after us)
    if (!mounted && !_mountRetryTimer) {
      _mountRetryTimer = setInterval(() => {
        if (mountSettingsPanel(_settingsPanel)) {
          clearInterval(_mountRetryTimer);
          _mountRetryTimer = null;
          updateSettingsPanel();
        }
      }, 300);
      // Give up after 30 seconds — append to body as last resort
      setTimeout(() => {
        if (_mountRetryTimer) {
          clearInterval(_mountRetryTimer);
          _mountRetryTimer = null;

...
(function () {
  ...
  function mountSettingsPanel(panel) {
    // TWSE mounts after #faction_war_list_id — that element is always present
    // on the faction page on all platforms (desktop, tablet, mobile), even when
    // the faction is not in a war. Mirror this exactly.
    const warList = document.getElementById("faction_war_list_id");
    if (warList) {
      // Place right after the TWSE panel if it's already there, otherwise after warList
      const twsePanel = document.querySelector("twse-settings-panel");
      const anchor = (twsePanel?.parentNode === warList.parentNode) ? twsePanel : warList;
      if (panel.previousElementSibling !== anchor) {
        anchor.after(panel);
      }
    } else {
      // #faction_war_list_id not in DOM yet — remove so it doesn't sit in the wrong place

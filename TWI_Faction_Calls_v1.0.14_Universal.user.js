...
(function () {
  ...
  function mountSettingsPanel(panel) {
    if (panel.isConnected) return; // already in the right place
    // Prefer: right after the TWSE settings <details>
    const twsePanel = document.querySelector("twse-settings-panel, .twse-settings-details, details.accordion[class*='twse']");
    if (twsePanel?.parentNode) {
      twsePanel.after(panel);
      return;
    }
    // Fallback: after #faction_war_list_id
    const warList = document.getElementById("faction_war_list_id");
    if (warList?.parentNode) {
      warList.after(panel);
      return;
    }
    // Last resort: append to #factions
  ...
  function injectSettingsPanel() {
    if (settingsPanelInjected) {
      // Re-mount if it got detached (e.g. after hash change)
  ...
  // Ensure settings panel is mounted on any factions.php page
  function ensureUI() {
    injectSettingsPanel();
    // Re-mount if it got displaced by TWSE loading after us
    const panel = document.getElementById("twi-settings-details");

...
(function () {
  ...
  // ── TWI Calls checkbox ─────────────────────────────────────────────────────
  // Primary anchor: inserted after the TWSE Settings <summary> (always present).
  // Fallback anchor: inserted before graphIcon in the war header (war page only).

  let toggleInjected = false;

  function injectToggle() {
    if (toggleInjected || document.getElementById("twi-calls-toggle-container")) {
      toggleInjected = true;
      return;
    }

    const wrap = document.createElement("span");
    wrap.id = "twi-calls-toggle-container";
    wrap.className = "twi-calls-inline-toggle";
    wrap.innerHTML = `
      <label class="twi-sort-toggle-label" style="display:inline-flex;align-items:center;gap:6px;cursor:pointer;font-size:13px;user-select:none;">
        <input type="checkbox" id="twi-calls-checkbox" class="twi-sort-toggle-checkbox" style="cursor:pointer;margin:0;width:13px;height:13px;" ${state.enabled ? "checked" : ""} />
        TWI Calls
      </label>`;

    // Option 1: next to the TWSE Settings summary — always present on faction page
    const twseSummary = document.querySelector(".twse-settings-details summary, details[class*='twse'] summary");
    if (twseSummary) {
      twseSummary.appendChild(wrap);
      toggleInjected = true;
    } else {
      // Option 2: before the graphIcon in the war header
      const graphContainer = document.querySelector('[class*="graphIcon"]');
      if (graphContainer?.parentNode) {
        graphContainer.parentNode.style.position = "relative";
        graphContainer.parentNode.insertBefore(wrap, graphContainer);
        toggleInjected = true;
      } else {
        // Option 3: append to the faction-war-wrap or any visible war header
        const warWrap = document.querySelector(".faction-war-wrap, .war-header, [class*='warHeader'], [class*='war-header']");
        if (warWrap) {
          warWrap.appendChild(wrap);
          toggleInjected = true;
        }
      }
    }

    if (!toggleInjected) return; // DOM not ready yet — will retry via observer

    wrap.querySelector("#twi-calls-checkbox").addEventListener("change", async (e) => {
  ...
  function updateToggleChecked() {
    const cb = document.getElementById("twi-calls-checkbox");
    if (cb) cb.checked = state.enabled;
  }

  // ── Settings panel (mirrors TWSE Settings accordion, uses Torn's own classes) ──
  // Injected after #faction_war_list_id, same placement as TWSE Settings panel.

  let settingsPanelInjected = false;

  function injectSettingsPanel(factionsContainer) {
    ...
    // Mount: after #faction_war_list_id when present, else directly inside #factions
    const mountPanel = () => {
      if (!panel.isConnected) {
        const warList = factionsContainer.querySelector("#faction_war_list_id");
        if (warList) {
          warList.after(panel);
        } else {
    ...
    const obs = new MutationObserver(mountPanel);
  ...
  // Inject toggle + settings on any factions.php page (war page or not)
  function ensureUI() {
    // Settings panel — inject on all factions.php pages
    if (!settingsPanelInjected) {
      const factionsContainer = document.getElementById("factions");
      if (factionsContainer) injectSettingsPanel(factionsContainer);
    }
    // Toggle checkbox — try multiple anchors
    if (!toggleInjected) injectToggle();
  }
  ...
  window.addEventListener("hashchange", async () => {
    ...
    // Keep settings panel across hash changes (already mounted in #factions)
    ensureUI();
    if (isWarPage()) {
      attachWarObserver();

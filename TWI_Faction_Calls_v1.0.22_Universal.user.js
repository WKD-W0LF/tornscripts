...
(function () {
  ...
  async function authenticate(force = false) {
    ...
    state.authenticating = (async () => {
      // If no API key is saved, prompt via the settings panel instead of a modal
      if (!state.apiKey) {
        await showAlert(
          "No API key set.\n\nPlease enter your 16-character Torn Public API key\n" +
          "in the TWI Faction Calls Settings panel below, then tick\n" +
          "the 'TWI Calls' checkbox again."
        );
        // Open the settings accordion so the user sees it
  ...
  // Copied from TWSE — waits for a DOM element using MutationObserver on
  // document.documentElement, which works on TornPDA, mobile and desktop.
  ...
  // Build the panel element once and return it — no DOM insertion here
  function buildSettingsPanel() {
    ...
    panel.querySelector("#twi-settings-save").addEventListener("click", async () => {
      ...
      if (state.enabled && (!wasEnabled || !validSession())) {
        ...
        if (!ok) { setEnabled(false); updateSettingsPanel(); updateToggleChecked(); return; }
        await refreshCalls();
        scheduleRender(0);
      } else if (!state.enabled && wasEnabled) {
        state.connected = false; state.calls.clear(); removeAllControls(); updateChip();
      }
      updateSettingsPanel();
    ...
    panel.querySelector("#twi-settings-forget").addEventListener("click", async () => {
      ...
      setApiKey(""); setEnabled(false); state.connected = false;
      state.calls.clear(); removeAllControls();
      updateSettingsPanel(); updateToggleChecked(); updateChip();
    });

    return panel;
  }

  // Place the panel into the page. Returns true if successfully placed in a
  // meaningful location (not just body fallback).
  function mountSettingsPanel(panel) {
    // 1. After twse-settings-panel — preferred anchor when TWSE is present
    const twsePanel = document.querySelector("twse-settings-panel");
    if (twsePanel?.isConnected) {
      if (panel.previousElementSibling !== twsePanel) {
        twsePanel.after(panel);
      }
      return true;
    }

    // 2. After #faction_war_list_id (the Chain panel) — fallback when TWSE absent
    //    But only place here if twse-settings-panel is NOT anywhere in the document
    //    yet; if TWSE is mid-load it will insert itself after warList and our
    //    MutationObserver will fire again, moving us to slot #1 above.
    const warList = document.getElementById("faction_war_list_id");
    if (warList?.isConnected) {
      if (panel.previousElementSibling !== warList) {
        warList.after(panel);
      }
      return true;
    }

    // 3. Last resort — append inside #factions container
    const factions = document.getElementById("factions");
    if (factions?.isConnected) {
  ...
  // The single panel instance, created once
  let _settingsPanel = null;

  // Mount the settings panel and keep it in the right position.
  // Priority: directly after twse-settings-panel (if TWSE is loaded),
  //           otherwise directly after #faction_war_list_id (the Chain panel).
  async function injectSettingsPanel() {
    if (_settingsPanel) {
      // Already created — reposition if needed (e.g. after tab switch)
      mountSettingsPanel(_settingsPanel);
      updateSettingsPanel();
      return;
    }

    // Wait for #factions just like TWSE does (works on TornPDA, iOS, desktop)
    const factionsContainer = await waitForElement("#factions");
    if (!factionsContainer) return; // timed out — not a factions page

    _settingsPanel = buildSettingsPanel();

    // checkAndMount is called every time the #factions subtree changes.
    // This means: if TWSE loads after us and inserts twse-settings-panel, our
    // next observer tick finds it and repositions us to sit right below it.
    let _mountScheduled = false;
    const checkAndMount = () => {
      if (_mountScheduled) return;
      _mountScheduled = true;
      // Defer one microtask so TWSE's own insertion fully settles before we read
      // previousElementSibling — avoids fighting over the same slot.
    ...
    obs.observe(factionsContainer, { childList: true, subtree: true });
    checkAndMount();
  }
  ...
  GM_addStyle(`
    ...
    /* ── Mobile (≤900px): control flows below name in flex column ── */
    @media(max-width:900px){
      .members-list li .member{
        display:flex!important;flex-direction:column!important;
        align-items:stretch!important;justify-content:center!important;
        min-height:60px!important;padding-top:4px!important;
        padding-bottom:4px!important;box-sizing:border-box!important
      }
      .members-list li .member>a{width:100%!important;min-width:0!important}
      .twi-call-control{
        display:flex!important;flex-wrap:nowrap!important;
        align-items:center!important;justify-content:flex-start!important;
        width:100%!important;margin:4px 0 0!important;
        padding:0 2px!important;gap:4px!important;box-sizing:border-box!important
      }
      .twi-call-main{
        min-width:68px!important;height:44px!important;min-height:44px!important;
        padding:4px 8px!important;gap:4px!important;
        font-size:11px!important;border-radius:6px!important
      }
      .twi-state-dot{width:10px!important;height:10px!important;min-width:10px!important}
      .twi-call-meta{max-width:60px!important;font-size:9px!important}
      .twi-call-actions{display:inline-flex!important;flex:0 0 auto!important;gap:4px!important}
      .twi-call-actions[hidden]{display:none!important}
      .twi-flag{
        width:44px!important;min-width:44px!important;
        height:44px!important;padding:0!important;border-radius:6px!important
      }
      .twi-flag-icon{width:18px!important;height:18px!important}
  ...
  // Called on every DOM mutation and hash change
  function ensureUI() {
    injectSettingsPanel();
    // If TWSE panel loaded after us, reposition ours to sit after it
    if (_settingsPanel?.isConnected) {
      const twsePanel = document.querySelector("twse-settings-panel");
      if (twsePanel?.isConnected && _settingsPanel.previousElementSibling !== twsePanel) {
        twsePanel.after(_settingsPanel);
        updateSettingsPanel();
      }
    }
  }

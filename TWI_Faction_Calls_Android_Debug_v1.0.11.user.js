...
// Removed @grant GM_xmlhttpRequest  → replaced with native fetch()
// Removed @grant GM_addStyle        → replaced with inline <style> injection
// Removed @grant GM_registerMenuCommand → not supported reliably in TornPDA
// Removed @connect                  → not needed without GM_xmlhttpRequest
// These changes eliminate the grey-arrow "restricted sandbox" state in TornPDA.
...
(function () {
  ...
  // ── HTTP — native fetch() replaces GM_xmlhttpRequest ─────────────────────
  // fetch() works cross-origin in TornPDA's WebView without any @grant or
  // @connect declarations. No grants = green monitor icon = script runs.
  ...
  async function request(method, path, body, authenticated = true) {
    ...
    try {
      ...
      clearTimeout(timer);
      if (err.name === "AbortError") throw new Error("Server request timed out");
      throw new Error("Unable to reach TWI Faction Calls server");
    }
    clearTimeout(timer);
